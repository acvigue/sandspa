    ...
    <q-list bordered separator>
      <q-item v-for="pattern in patterns" :key="pattern.id">
        ...
        <q-item-section avatar>
          <img height="75px" :src="'https://webcenter.sisyphus-industries.com/uploads/track/thr/' + pattern.db_tracks[pattern.featured_track] + '/thumb_400.png'">
        </q-item-section>
        <q-item-section>
          <q-item-label class="home__item-title">{{pattern.name}}</q-item-label>
          <q-item-label class="home__item-title text-grey-7">{{pattern.description}}</q-item-label>
...
export default {
  ...
  methods: {
    ...
    refreshFiles: async function(done) {
      ...
      //Get popular tracks
      await axios.get("https://webcenter.sisyphus-industries.com/playlists.json?is_featured=false", { headers: {Authorization: this.$root.community.authToken }}).then((response) => {
