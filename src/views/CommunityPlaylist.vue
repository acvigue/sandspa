  ...
  <div class="home">
    <q-header elevated class="header bg-black text-white shadow-0">
      <q-toolbar>
        ...
        <q-toolbar-title class="text-center">{{ $route.params.name }}</q-toolbar-title>
        <q-btn flat round dense icon="get_app" class="q-mr-sm" @click="downloadPlaylist()"/>
      </q-toolbar>
    </q-header>
...
export default {
  ...
  methods: {
    downloadPlaylist: async function() {
      ...
      for(let i = 0; i < this.apatterns.length; i++) {
        const pattern = this.apatterns[i];
        seqFile += pattern.track_id + "-" + pattern.name.replace(/-/g, "") + ".thr\n";
      }

      var formData = new FormData();
      var blob = new Blob([seqFile], {type: "text/plain"});
      formData.append("file", blob, this.$route.params.id + "-" + this.$route.params.name.replace(/-/g, "") + ".seq");
    ...
    refreshFiles: async function(done) {
      ...
      //Get popular tracks
      await axios.get("https://webcenter.sisyphus-industries.com/playlists/" + this.$route.params.id + ".json", { headers: {Authorization: this.$root.community.authToken }}).then((response) => {
