...
export default {
  ...
  methods: {
    downloadPattern: async function(pattern) {
      ...
      formData.append("file", blob, pattern.track_id + "-" + pattern.name + ".thr");
    ...
    refreshFiles: async function(done) {
      ...
      //Get popular tracks
      await axios.get("https://webcenter.sisyphus-industries.com/tracks.json?sort=newest_designs", { headers: {Authorization: this.$root.community.authToken }}).then((response) => {
