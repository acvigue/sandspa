<template>
  <div class="home">
    ...
    <q-pull-to-refresh color="white" bg-color="black" @refresh="refreshFiles">
        ...
        <!-- Featured Playlists -->
        <q-carousel class="community__carousel rounded-borders" autoplay slide animated v-model="slide" arrows infinite>
            <q-carousel-slide @click="openPlaylist(playlist.name, playlist.playlist_id)" :style="'background-color: ' + playlist.bg_color" v-for="(playlist, index) in featured_playlists" :key="playlist.id" :name="index" :img-src="playlist.thumburl">
        ...
        <q-list bordered separator>
            <q-item @click.native="$router.push('/community/popular')" class="community__listitem" clickable v-ripple>
            ...
            <q-item @click.native="$router.push('/community/newest')" class="community__listitem" clickable v-ripple>
                <q-item-section avatar>
                    <q-icon style="font-size: 40px;" color="black" name="album" />
                </q-item-section>
            ...
            <q-item @click.native="$router.push('/community/playlists')" class="community__listitem" clickable v-ripple>
                <q-item-section avatar>
                    <q-icon style="font-size: 40px;" color="black" name="format_list_bulleted" />
                </q-item-section>
...
export default {
  ...
  methods: {
    ...
    refreshFiles: async function(done) {
      ...
      await axios.get("https://webcenter.sisyphus-industries.com/playlists.json", { headers: {Authorization: this.$root.community.authToken }}).then((response) => {
        const playlists = response.data.resp;
        playlists.forEach(playlist => {
            if(playlist.is_featured) {
                playlist.thumburl = "https://webcenter.sisyphus-industries.com/uploads/track/thr/" + playlist.db_tracks[playlist.featured_track] + "/thumb_400.png";
